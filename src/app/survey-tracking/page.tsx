...
function computeStats(businesses: Business[]) {
  ...
  // Fixed order with zero-count results kept, so donut arcs keep stable
  // keys and positions and can sweep between states when cross-filtering.
      ...
      <div className="flex flex-col gap-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <StatCard
            ...
            value={<span className="num text-mid"><AnimatedNumber value={stats.pending.length} /></span>}
            icon={<Clock size={16} className="text-subtle" strokeWidth={1.8} />}
          />
          <StatCard
            label="דווחו מהשטח"
            value={<span className="num"><AnimatedNumber value={stats.reported.length} /></span>}
            icon={<ClipboardCheck size={16} className="text-subtle" strokeWidth={1.8} />}
            footer={<span className="text-[12px] text-subtle"><span className="num"><AnimatedNumber value={stats.completionPct} suffix="%" /></span> שיעור השלמה</span>}
          />
          <StatCard
            tone="brand"
          ...
          {donut.reported.length === 0 ? (
            ...
            <div className="flex items-center gap-6 max-w-sm">
              <svg width="112" height="112" viewBox="0 0 42 42" className="shrink-0">
                ...
                {arcs.map(arc => {
                  const item = chartItemProps(cf, 'result', arc.key, `${arc.label}: ${arc.count}`)
                  return (
                  <circle
                    key={arc.key}
                    {...item}
                ...
                <text x="21" y="20.4" textAnchor="middle" style={{ font: "700 6px var(--font-num)", fill: '#0f1a28' }}><AnimatedNumber value={donut.reported.length} /></text>
        ...
        <Card padded={false} className="flex-1 min-h-0">
          <div className="flex items-center justify-between px-5 py-4 border-b border-hairline">
            ...
            <span className="num text-[12.5px] text-subtle"><AnimatedNumber value={stats.pending.length} /></span>
          </div>
